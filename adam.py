...
axs[1].set_title('Accuracy and Runtime vs. Batches')
...
for betas in betas_list:
    ...
    iterations = np.arange(0, len(model.costs) * 1000, 1000)
    label = f'Batch Size: {model.m}'
    axs[0].plot(iterations, model.costs, label=label)

axs[0].legend()

axs[1].plot(range(len(betas_list)), runtimes, label='Runtime', marker='o')
axs[1].set_ylabel('Runtime (seconds)', color='tab:blue')
axs[1].tick_params(axis='y', labelcolor='tab:blue')

axs2 = axs[1].twinx()
axs2.plot(range(len(betas_list)), accuracies, color='tab:red', label='Accuracy', marker='o')
axs2.set_xticks(range(len(betas_list)))
